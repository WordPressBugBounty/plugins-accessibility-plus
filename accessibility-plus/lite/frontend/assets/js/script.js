...
const metaViewportScalableFixes = (() => {
  ...
  const fixViewportContent = (content) => {
    ...
    const validParts = parts.filter(p => {
      ...
      // Only remove user-scalable=no or user-scalable=0 (not yes or 1)
      if (p.match(/user\-scalable\s*=\s*(no|0)\b/i)) return false;
      // Only remove maximum-scale=1.0 or maximum-scale=1 (prevents scaling)
      // Preserve maximum-scale > 1.0 (e.g., maximum-scale=2.0 is fine)
      if (p.match(/maximum\-scale\s*=\s*1\.?0*\b/i)) return false;
      return true;
    });

    // Add required parts if missing (don't override existing values)
  ...
  if (existingViewport) {
    ...
    // This is safe - standard viewport tag that all responsive sites should have
